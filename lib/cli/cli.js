...
.option(
    '-X, --existdb',
    'Support existdb syntax'
)
.option(
    '-C, --precommit',
    'Output for precommit hook'
)
...
.action(function(p) {
    ...
    files.forEach(function(file){
        ...
        var linter = new XQLint(source, { fileName: file, styleCheck: lintCmd.styleCheck, processor: lintCmd.processor, existdb: lintCmd.existdb });
        var markers = linter.getMarkers().sort(function(a, b){ return a.sl - b.sl; });
        var lines = source.split('\n');
        if(markers.length !== 0) {
	    if (!lintCmd.emacs && !lintCmd.precommit) {
		console.log(('\n' + file).bold);
		...
		linter.getWarnings().forEach(function(error){
            ...
            } else if (!lintCmd.emacs) { // --precommit
                linter.getErrors().forEach(function(){
                    errors++;
                });
                linter.getWarnings().forEach(function(){
                    warnings++;
                });
                markers.forEach(function(marker){
                    console.error(marker.type.toUpperCase() + ': ' + file + ':' + (marker.pos.sl + 1) + ':' + (marker.pos.sc) + ':' + marker.type + ' ' + marker.message);
                });
            } else { // --emacs
		markers.forEach(function(marker){
    ...
    if (lintCmd.precommit) {
    if(errors === 0 && warnings === 0) {
        // no news is good news
    } else if(errors > 0) {
        console.error(('Found ' + errors + ' errors and ' + warnings + ' warnings.').red);
        process.exit(errors);
    } else if(warnings > 0) {
        console.log(('Found ' + errors + ' errors and ' + warnings + ' warnings.').yellow);
    }
    }
});
