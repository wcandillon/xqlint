...
var ExistParser = require('./parsers/ExistParser').ExistParser;
var ExistdbParseTreeHandler = require('./parsers/ExistdbParseTreeHandler').ExistdbParseTreeHandler;
...
exports.JSONParseTreeHandler = JSONParseTreeHandler;
exports.XQLint = function (source, opts) {

    //ACE editor worker hack
    if(_.defaults) {
        opts = _.defaults(opts ? opts : {}, { styleCheck: false, existdb: false });
    }
    ...
    var h = opts.existdb === true ? new ExistdbParseTreeHandler(source) : new JSONParseTreeHandler(source);
    var parser = opts.existdb === true ? new ExistParser(source, h) : (isJSONiq ? new JSONiqParser(source, h) : new XQueryParser(source, h));
    try {
        parser.parse_XQuery();
    } catch (e) {
