exports.Translator = function(rootStcx, ast){
    ...
    this.WS = function(node){
        ...
        var isComment = value.substring(0, '(:'.length) === '(:' && value.substring(value.length - ':)'.length) === ':)';
        if(isComment) {
            var comment = value.substring('(:'.length);
            comment = comment.substring(0, comment.length - ':)'.length).trim();
            var isXQLint = comment.substring(0, 'xqlint'.length) === 'xqlint';
            if(isXQLint) {
                var commands = comment.match(/[a-zA-Z]+\(([^)]+)\)/g);
                commands.forEach(function(command){
                    var name = command.substring(0, command.indexOf('('));
                    var args = command.substring(0, command.length - 1).substring(command.indexOf('(') + 1).split(',').map(function(val){ return val.trim(); });
                    if(name === 'varrefs') {
