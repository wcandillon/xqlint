...
(:~
 ...
 :
 :)
module namespace xmlrpc = "http://www.28msec.com/modules/xmlrpc";
...
(:~
 ...
 :)
declare %ann:sequential function xmlrpc:invoke(
  $endpoint-url as xs:string,
  $method as xs:string,
  $parameters as item()*)
as item()*
{ 
  variable $request as element(http:request) := <http:request href="{$endpoint-url}" method="POST" />;
...
(:~
 ...
 : @return Result of the method invocation.
 :)
declare %ann:sequential function xmlrpc:invoke($endpoint-url as xs:string, $method as xs:string) as item()*
{
...
declare %private %ann:sequential function xmlrpc:set-content-type($request as element(http:request)) as element(http:request)
{
...
declare %private function xmlrpc:result($body as node()) as item()*
{
